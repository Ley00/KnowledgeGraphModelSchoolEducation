...
def write_history_comparison(
    ...
    paths = PipelinePaths.from_project_root(project_root, mode=mode)
    paths.output_dir.mkdir(parents=True, exist_ok=True)

    comparison = compare_min_history(paths.project_root, history_values=history_values, mode=mode)
    csv_path = paths.output_dir / "history_min_comparison.csv"
    txt_path = paths.output_dir / "history_min_comparison.txt"
    ...
    if successful.empty:
        ...
        lines.append(
            f"- melhor regressao por MAE: min_history={int(best_regression['min_history'])} ({best_regression['regression_mae']:.4f})"
        )
        lines.append(
            f"- melhor classificacao por F1: min_history={int(best_classification['min_history'])} ({best_classification['classification_f1']:.4f})"
        )
