...
def prepare_teachers(df: pandas.DataFrame) -> pandas.DataFrame:
    ...
    if df is None or df.empty:
        return pandas.DataFrame(
            columns=[
                "IDUnidade", "IDPeriodo", "IDCurso", "IDSerie", "IDTurma", "IDDisciplina",
                "NomeFuncionario", "quantidade_professores_disciplina"
            ]
        )
    ...
    teachers["IDSerie"] = teachers["IDSerie"].astype(str)
    teachers["IDTurma"] = teachers["IDTurma"].astype(str)
    teachers["IDDisciplina"] = teachers["IDDisciplina"].astype(str)

    grouped = (
        teachers.groupby(["IDUnidade", "IDPeriodo", "IDCurso", "IDSerie", "IDTurma", "IDDisciplina"], dropna=False)
        .agg(
