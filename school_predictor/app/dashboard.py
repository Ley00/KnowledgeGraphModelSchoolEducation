...
REPORT_DIR = DEFAULT_PATHS.reports_artifacts_dir
...
def render_coordination() -> None:
    ...
    ranking = safe_read("ranking_executivo_coordenacao.csv")
    integrated = safe_read("relatorio_escolar_integrado.csv")
    teacher_by_class = safe_read("relatorio_professores_por_turma.csv")
    teacher_overall = safe_read("relatorio_professores_geral.csv")
    ...
    teacher_by_class = normalize_filters(teacher_by_class, key_prefix="coord_teacher_class")
    teacher_overall = normalize_filters(teacher_overall, key_prefix="coord_teacher_overall")
    ...
    st.subheader("Professores por turma")
    metric_row(
        [
            ("Grupos professor/turma", f"{len(teacher_by_class):,}".replace(",", ".")),
            ("Alunos em risco", f"{teacher_by_class['alunos_com_risco'].sum():,}".replace(",", ".")),
            ("Casos altos", f"{teacher_by_class['casos_alto_risco'].sum():,}".replace(",", ".")),
            ("Sem professor vinculado", f"{int((teacher_by_class['professor_disponivel'] == 0).sum()):,}".replace(",", ".")),
        ]
    )

    top_teacher_by_class = teacher_by_class.sort_values(
        ["alunos_com_risco", "casos_alto_risco", "casos_risco_moderado"],
        ascending=[False, False, False],
    ).head(20)
    fig_teacher_by_class = px.bar(
        top_teacher_by_class,
        x="alunos_com_risco",
        y="professor_responsavel",
        color="NomeSerie",
        orientation="h",
        title="Professores por turma com mais alunos em risco",
        hover_data=["NomeCurso", "ApelidoTurma", "casos_alto_risco", "casos_risco_moderado", "casos_baixo_risco"],
    )
    fig_teacher_by_class.update_layout(yaxis={"categoryorder": "total ascending"})
    st.plotly_chart(fig_teacher_by_class, use_container_width=True)
    ...
    st.subheader("Professores no período")
    metric_row(
        [
            ("Professores no recorte", f"{len(teacher_overall):,}".replace(",", ".")),
            ("Alunos em risco", f"{teacher_overall['alunos_com_risco'].sum():,}".replace(",", ".")),
            ("Casos altos", f"{teacher_overall['casos_alto_risco'].sum():,}".replace(",", ".")),
            ("Sem professor vinculado", f"{int((teacher_overall['professor_disponivel'] == 0).sum()):,}".replace(",", ".")),
        ]
    )

    top_teacher_overall = teacher_overall.sort_values(
        ["alunos_com_risco", "casos_alto_risco", "casos_risco_moderado"],
        ascending=[False, False, False],
    ).head(20)
    fig_teacher_overall = px.bar(
        top_teacher_overall,
        x="alunos_com_risco",
        y="professor_responsavel",
        color="NomePeriodo",
        orientation="h",
        title="Professores com mais alunos em risco no período",
        hover_data=["alunos_monitorados", "casos_alto_risco", "casos_risco_moderado", "casos_baixo_risco"],
    )
    fig_teacher_overall.update_layout(yaxis={"categoryorder": "total ascending"})
    st.plotly_chart(fig_teacher_overall, use_container_width=True)
