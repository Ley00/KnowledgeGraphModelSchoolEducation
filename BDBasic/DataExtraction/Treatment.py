    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            if isinstance(params, dict):
                params = {k: (None if (isinstance(v, float) and math.isnan(v)) or v is numpy.nan else v) for k, v in params.items()}
            elif isinstance(params, (list, tuple)):
                params = tuple(None if (isinstance(v, float) and math.isnan(v)) or v is numpy.nan else v for v in params)
            result = session.execute(query, params)
