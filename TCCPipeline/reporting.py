...
def _load_mode_artifacts(project_root: str | Path, mode: str) -> tuple[pandas.DataFrame, pandas.DataFrame]:
    ...
    base_dir = root / "TCCPipeline" / "Result" / mode
...
def _prepare_combined_predictions(project_root: str | Path) -> pandas.DataFrame:
    ...
    combined["professor_responsavel"] = combined.get("NomeFuncionario", pandas.Series(dtype=str)).fillna("Professor não identificado")
...
def _write_text_summaries(
    ...
    (output_dir / "relatorio_professor.txt").write_text("\n".join(teacher_lines), encoding="utf-8")
    (output_dir / "relatorio_coordenacao.txt").write_text("\n".join(coordinator_lines), encoding="utf-8")
    (output_dir / "relatorio_secretaria.txt").write_text("\n".join(secretary_lines), encoding="utf-8")
    (output_dir / "ranking_executivo_coordenacao.txt").write_text("\n".join(executive_lines), encoding="utf-8")
...
def build_school_reports(project_root: str | Path = ".") -> dict[str, Path]:
    ...
    """
    root = Path(project_root).resolve()
    output_dir = root / "TCCPipeline" / "Result" / "relatorios_escolares"
    output_dir.mkdir(parents=True, exist_ok=True)

    combined = _prepare_combined_predictions(root)
    ...
    combined.to_csv(output_dir / "relatorio_escolar_integrado.csv", index=False, encoding="utf-8")
    teacher_report.to_csv(output_dir / "relatorio_professor.csv", index=False, encoding="utf-8")
    coordinator_report.to_csv(output_dir / "relatorio_coordenacao.csv", index=False, encoding="utf-8")
    secretary_report.to_csv(output_dir / "relatorio_secretaria.csv", index=False, encoding="utf-8")
    executive_ranking.to_csv(output_dir / "ranking_executivo_coordenacao.csv", index=False, encoding="utf-8")
    _write_text_summaries(output_dir, teacher_report, coordinator_report, secretary_report, executive_ranking)
