...
from TCCPipeline.data import prepare_absences, prepare_grades, prepare_payments, prepare_teachers
...
def build_prediction_dataset(source_tables: dict[str, pandas.DataFrame], min_history: int = 2) -> pandas.DataFrame:
    ...
    dataset = dataset.merge(
        teachers,
        on=["IDUnidade", "IDPeriodo", "IDCurso", "IDDisciplina"],
        how="left",
    )
    ...
    dataset["NomeFuncionario"] = dataset["NomeFuncionario"].fillna("Professor não identificado")
