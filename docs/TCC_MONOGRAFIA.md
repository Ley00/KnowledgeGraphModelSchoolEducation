...
orientador: "Otávio Xavier Calaça"
cidade: "Goiânia"
ano: "2026"
titulo: "Sistema preditivo de desempenho escolar para apoio à decisão pedagógica"
subtitulo: "Uma pipeline temporal para previsão de notas, identificação de risco acadêmico e geração de relatórios escolares"
---

## CAPA

<div style="text-align: center;">
  <p><strong>Instituto Federal de Educação, Ciência e Tecnologia de Goiás</strong></p>
  <p>Pró-Reitoria de Ensino</p>
  <p>Câmpus Goiânia</p>
  <p>Graduação em Sistemas de Informação</p>
</div>

<div style="height: 110px;"></div>

<div style="text-align: center;">
  <p><strong>Warley Rodrigues de Andrade</strong></p>
</div>

<div style="height: 110px;"></div>

<div style="width: 72%; margin: 0 auto; text-align: left;">
  <p style="font-size: 1.8em; font-weight: 700; line-height: 1.18; margin: 0;">
    Sistema preditivo de desempenho escolar para apoio à decisão pedagógica
  </p>
  <p style="font-size: 1.2em; line-height: 1.35; margin-top: 18px;">
    Uma pipeline temporal para previsão de notas, identificação de risco acadêmico e geração de relatórios escolares
  </p>
</div>

<div style="height: 90px;"></div>

<div style="width: 42%; margin-left: auto; margin-right: 10%;">
  <p style="text-align: left; line-height: 1.7;">
    Trabalho de Conclusão de Curso apresentado ao curso de Graduação em Sistemas de Informação do Instituto Federal de Goiás, Câmpus Goiânia, como requisito parcial para obtenção do título de Bacharel em Sistemas de Informação.
  </p>
  <p style="text-align: left;">
    <strong>Orientador:</strong> Prof. Otávio Xavier Calaça
  </p>
</div>
...
<div style="text-align: center;">
  <p><strong>Warley Rodrigues de Andrade</strong></p>
  <br /><br />
  <p><strong>SISTEMA PREDITIVO DE DESEMPENHO ESCOLAR PARA APOIO À DECISÃO PEDAGÓGICA</strong></p>
  <p>Uma pipeline temporal para previsão de notas, identificação de risco acadêmico e geração de relatórios escolares</p>
</div>
...
Orientador: Prof. Otávio Xavier Calaça
...
Após a constatação de que a base pivotada e a estrutura experimental não eram suficientes para o objetivo institucional do TCC, foi implementada primeiro uma pipeline modular intermediária e, em seguida, a arquitetura final consolidada no pacote `school_predictor/`.
...
O amadurecimento do projeto culminou na criação de uma arquitetura final centrada em `school_predictor/`, com separação entre:

- acesso ao banco e variáveis de ambiente;
- preparação física e anonimização do banco restaurado;
- extração privada local dos dados;
- interface pública baseada em arquivos CSV canônicos;
- pipeline de modelagem e relatórios;
- dashboard para visualização operacional.

Essa refatoração representou o ponto de consolidação do TCC, transformando uma sequência de experimentos em uma pipeline reproduzível de ponta a ponta, mais adequada à publicação do repositório sem expor a estrutura interna do banco institucional.
...
- `school_predictor/database/access.py`: conexão com o banco configurada por ambiente local;
- `school_predictor/database/maintenance.py`: wrapper público da preparação do banco;
- `school_predictor/database/extraction.py`: wrapper público da extração;
- `school_predictor/database/private_runtime.py`: camada privada local com a rotina física do banco e a extração com tratamento sensível;
- `school_predictor/database/private_sql/`: consultas SQL reais mantidas apenas no ambiente local;
- `school_predictor/pipeline/`: leitura das fontes, construção do dataset, modelagem, avaliação e relatórios;
- `school_predictor/app/dashboard.py`: dashboard em Streamlit;
- `artifacts/`: diretório canônico de CSVs, artefatos de modelagem e relatórios operacionais.
...
- salvar artefatos e relatórios em diretórios canônicos sob `artifacts/`;
- separar explicitamente a camada privada de banco da camada pública reproduzível por CSVs;
...
O repositório foi organizado em cinco blocos principais:

- `school_predictor`: base ativa da aplicação final;
- `artifacts`: diretório canônico das entradas e saídas locais;
- `docs`: documentação textual do projeto e da monografia;
- `monografia`: projeto LaTeX da monografia e do pré-projeto;
- `diagrama`: diagramas da arquitetura e do fluxo operacional.

## APÊNDICE B - PRINCIPAIS ARQUIVOS DA PIPELINE FINAL

- `school_predictor/database/access.py`
...
- `school_predictor/pipeline/dataset.py`
- `school_predictor/pipeline/modeling.py`
- `school_predictor/pipeline/reporting.py`
...
- `artifacts/pipeline/previsao_nota/student_prediction_report.txt`
- `artifacts/pipeline/alerta_risco/student_prediction_report.txt`
- `artifacts/reports/relatorio_professor.txt`
- `artifacts/reports/relatorio_coordenacao.txt`
- `artifacts/reports/relatorio_secretaria.txt`
- `artifacts/reports/ranking_executivo_coordenacao.txt`
